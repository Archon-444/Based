...
const StatCard: React.FC<{ icon: React.ReactNode; label: string; value: string; glow?: string }> = ({
    ...
    <motion.div
        ...
        className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#141B3D]/70 p-6 backdrop-blur-md"
    ...
    >
        ...
        <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-[#00D4FF] to-[#6B4CE6] opacity-40" />
    </motion.div>
);
...
const ReputationBar: React.FC<{ value: number }> = ({ value }) => (
    <div className="flex items-center gap-3">
        ...
        <span className="text-xs font-mono text-gray-300 min-w-[36px] text-right">{value}</span>
    </div>
);
        ...
        <div className="min-h-screen bg-[#050713] text-white">
            ...
            <div className="relative overflow-hidden border-b border-white/5">
                <div
                    ...
                    style={{
                        background:
                            'radial-gradient(ellipse at 70% 50%, rgba(107,76,230,0.5) 0%, transparent 60%), radial-gradient(ellipse at 30% 50%, rgba(0,212,255,0.4) 0%, transparent 60%)',
                    }}
                />
                <PremiumContainer size="xl">
                    <div className="py-14 relative z-10">
                        ...
                        <p className="text-lg text-gray-400 max-w-2xl">
                            ...
                            truth providers securing MoveMarket's prediction outcomes.
                        </p>
                    </div>
                </PremiumContainer>
            </div>

            <PremiumContainer size="xl" className="py-10 space-y-10">
                {/* Stats Row */}
                ...
                {activeTab === 'leaderboard' && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                        <div className="rounded-2xl border border-white/10 overflow-hidden bg-[#0F1530]/80 backdrop-blur-md">
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="text-xs font-semibold uppercase tracking-wider text-gray-500 border-b border-white/5">
                                    ...
                                    {MOCK_LEADERBOARD.map((oracle, i) => (
                                        ...
                                        >
                                            <td className="px-6 py-4">
                                                {oracle.rank <= 3 ? (
                                                    <span className={`text-lg font-bold ${oracle.rank === 1 ? 'text-yellow-400' : oracle.rank === 2 ? 'text-gray-300' : 'text-orange-400'}`}>
                                                        #{oracle.rank}
                                                    </span>
                                                ) : (
                                                    <span className="text-gray-500">#{oracle.rank}</span>
                                                )}
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="font-semibold text-white">{oracle.name}</div>
                                                <div className="text-xs font-mono text-gray-500">{oracle.address}</div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 hidden lg:table-cell text-gray-300 font-mono text-sm">
                                                {oracle.stake}
                                            </td>
                                            <td className="px-6 py-4">
                        ...
                        {MOCK_PENDING_MARKETS.map((market, i) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className="rounded-2xl border border-white/10 bg-[#141B3D]/70 p-6 backdrop-blur-md hover:border-white/20 transition-all group"
                                >
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-2">
                                                <span className="text-xs font-mono text-gray-500 bg-black/30 px-2 py-0.5 rounded border border-white/5">#{market.id}</span>
                                                {isUrgent && (
                ...
                {activeTab === 'register' && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-xl">
                        <div className="rounded-2xl border border-white/10 bg-[#141B3D]/70 p-8 backdrop-blur-md">
                            ...
                            <div className="space-y-4 mb-6">
                                <div className="flex items-start gap-3 p-4 rounded-xl bg-black/20 border border-white/5">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-400">Stake acts as collateral, slashed 20% for incorrect votes.</div>
                                    </div>
                                </div>
                                <div className="flex items-start gap-3 p-4 rounded-xl bg-black/20 border border-white/5">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-400">All submissions must be signed to prevent oracle impersonation.</div>
                                    </div>
                                </div>
                                <div className="flex items-start gap-3 p-4 rounded-xl bg-black/20 border border-white/5">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-400">+10 reputation per correct vote. Higher rep = more voting weight.</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm text-gray-400 mb-2">Stake Amount (APT)</label>
                                    <div className="relative">
                                        ...
                                        />
                                        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-500 text-sm">APT</span>
                                    </div>
                                </div>
                                <div>
            ...
            {voteModal !== null && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
                    <motion.div
                        ...
                        className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0F1530] p-8 shadow-[0_0_50px_rgba(0,212,255,0.1)]"
                    >
                        <div className="flex items-center gap-3 mb-6">
                            <div className="p-2.5 rounded-xl bg-gradient-to-br from-[#00D4FF] to-[#6B4CE6]">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-400">Market #{voteModal}</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm text-gray-400 mb-2">Evidence Hash (SHA-256)</label>
                                <input
                                    type="text"
                                    placeholder="0x..."
                        ...
                        <div className="flex gap-3 mt-6">
                            <button
                                ...
                                className="flex-1 py-3 rounded-xl border border-white/10 text-gray-400 hover:text-white hover:border-white/20 transition-all text-sm font-medium"
                            >
                                Cancel
                            </button>
                            <button
